::CrockPot.HooksMod.hook("scripts/factions/actions/send_beast_roamers_action", function(q) {
	...
	q.onUpdate = @(__original) function( _faction )
	{
		...
		// This action is now completely dormant during the first few days of each campaign
		if (::World.getTime().Days < ::CrockPot.Const.DayThresholdMinDangerBeasts) return;

		// We enforce the maximum beasts on the map via a member variable now
		if (this.getSpawnedParties(_faction).len() > this.m.MaxSpawnedParties) return;
